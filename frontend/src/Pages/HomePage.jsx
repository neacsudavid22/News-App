...
const HomePage = () => {
  ...
  useEffect( () => {
    const handleArticles = async () => { 
      try {
          const data = (toModify === true && user !== null) ? await getAuthorsArticles(user._id) : await getArticles(category);
  ...
  return (
    <Stack direction="vertical" className="w-100 bg-light">
      ...
      <div className="fixed-top">
        <MainNavbar />
        {!toModify && <CategoryBar category={category} setCategory={setCategory}></CategoryBar>}
      </div>
